...
\begin{verbatim}
Toilet(?t), integer[>=30, <=100](?distance),
hasDistance(?t, ?distance) -> hasGoodRange(?t, true)

DPSafeRule
    (Annotation(rdfs:comment "Rule with data range restriction")
    Body(
        ClassAtom(:Toilet Variable(var:t))
        DataPropertyAtom(:hasDistance Variable(var:t) Variable(var:distance))
        DataRangeAtom(
            DatatypeRestriction(xsd:integer xsd:minInclusive "30"^^xsd:integer
                xsd:maxInclusive "100"^^xsd:integer) Variable(var:distance)) )
    Head(
        DataPropertyAtom(:hasGoodRange Variable(var:t "true"^^xsd:boolean)
...
\begin{lstlisting}[language={swrl}, caption={SWRL example}, label={Script}]
Toilet(?t), integer[>=30, <=100](?distance),
hasDistance(?t, ?distance) -> hasGoodRange(?t, true)
